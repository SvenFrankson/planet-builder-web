...
class PlanetChunckGroup extends AbstractPlanetChunck {
    ...
    constructor(iPos, jPos, kPos, planetSide, level) {
        ...
        this._barycenter = PlanetTools.EvaluateVertex(this.size, PlanetTools.CHUNCKSIZE * (this.iPos * levelCoef + 0.5), PlanetTools.CHUNCKSIZE * (this.jPos * levelCoef + 0.5)).scale(PlanetTools.KGlobalToAltitude((this.kOffsetNext + this.kPos * levelCoef + 0.5) * PlanetTools.CHUNCKSIZE));
        this._barycenter = BABYLON.Vector3.TransformCoordinates(this._barycenter, planetSide.computeWorldMatrix(true));
    }
    subdivide() {
        for (let i = 0; i < 2; i++) {
