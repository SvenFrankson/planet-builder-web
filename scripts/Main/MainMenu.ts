...
class MainMenu extends Main {
    ...
    public async initialize(): Promise<void> {
		...
		return new Promise<void>(resolve => {
			...
			let mainMenuPlanet: Planet = PlanetGeneratorFactory.Create(this.currentGalaxy, BABYLON.Vector3.Zero(), PlanetGeneratorType.Earth, 4, this);
	...
	public async generatePlanets(): Promise<void> {
		...
		for (let i = 0; i < orbitCount; i++) {
			let planet: Planet = PlanetGeneratorFactory.Create(this.currentGalaxy, new BABYLON.Vector3(Math.cos(alpha) * orbitRadius * (i + 1), 0, Math.sin(alpha) * orbitRadius * (i + 1)), i + 1, 4, this);
